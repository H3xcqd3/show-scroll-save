...
const Films = () => {
  ...
  const items = library.filter(l => l.mediaType === 'movie');
    ...
    <>
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 space-y-6">
        <div>
          ...
          <p className="text-muted-foreground">Your saved movies</p>
        </div>
          ...
          <div className="space-y-3">
            <AnimatePresence mode="popLayout">
              {items.map((item: LibraryItem) => (
                ...
                >
                  <Link to={`/movie/${item.id}`} className="shrink-0">
                    {item.posterPath ? (
                  ...
                  <div className="flex-1 min-w-0">
                    <Link to={`/movie/${item.id}`} className="hover:underline">
